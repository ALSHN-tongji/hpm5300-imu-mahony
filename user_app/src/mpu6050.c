                         ...
                         mpu6050_dlpf_t dlpf)
{
    ...
    /* MPU6050 typical zero-rate tempco: ~0.02% FS/°C ≈ 0.05 °/s/°C at ±250dps.
     * X/Y are in-plane (coplanar), Z is out-of-plane — Z usually has higher
     * thermal sensitivity due to asymmetric MEMS structure and PCB stress.
     * Online calib will refine all axes once enough data is collected. */
    ...
    /* DMA IRQ enabled later in start_acquisition() —
     * Calibration uses ISR-driven DMA (same chain as main loop). */

    return status_success;
}

/* ── Start ISR-driven acquisition (CPU-decoupled, autonomous DMA loop) ── */
...
/* Change DLPF bandwidth at runtime. Used by MODE 7 DLPF auto-sweep.
 * Writes to CONFIG register (0x1A) directly. DMA acquisition continues
 * undisturbed — DLPF change takes effect on next sample. */
hpm_stat_t mpu6050_set_dlpf(mpu6050_t *dev, mpu6050_dlpf_t dlpf)
{
    return imu_write_reg(dev->i2c_ctx->base, dev->dev_addr,
                         MPU6050_REG_CONFIG, (uint8_t)dlpf);
}

/* Read back CONFIG register DLPF_CFG bits (0-6). For verifying a runtime
 * DLPF change actually landed on the sensor (MODE 7 sweep diagnostics). */
hpm_stat_t mpu6050_get_dlpf(mpu6050_t *dev, uint8_t *dlpf)
{
    uint8_t cfg = 0;
    hpm_stat_t stat = imu_read_regs(dev->i2c_ctx->base, dev->dev_addr,
                                    MPU6050_REG_CONFIG, &cfg, 1);
    if (stat == status_success) *dlpf = (uint8_t)(cfg & 0x07);
    return stat;
}

/* Read magnetometer from MPU-9250 EXT_SENS_DATA_00 (I2C Master mode).
 * MPU-9250's internal I2C master reads AK8963 automatically and
 * stores 6 bytes at registers 0x49-0x4E. Sensitivity = 0.15 μT/LSB. */
...
/* ========================================================================
 * Calibration (ISR-driven DMA, same chain as main loop)
 * ======================================================================== */
void mpu6050_calib_gyro_raw(mpu6050_t *dev, mpu6050_ring_t *ring, int raw_samples)
{
    ...
    /* Use ISR-driven DMA (same proven chain as main loop) */
    printf("#  calib: probing...\n");
    ring->head = ring->tail = 0;
    dev->dma_error = false;
    mpu6050_start_acquisition(dev, ring);

    /* Wait for first frame (2s timeout) */
    {
    ...
    printf("# Gyro off@T0 (raw): %d, %d, %d\n",
           dev->gyro_off_at_T0[0],
           dev->gyro_off_at_T0[1],
           dev->gyro_off_at_T0[2]);
 ...
 *
 * User places IMU in 6 orientations (Z±/X±/Y±), collects N samples each.
 * Computes offset (OX,OY,OZ) and scale (SX,SY,SZ) to correct accel to ±1g.
 *
 * Model:  a_cal = S * (raw - O) / accel_sf
 * ======================================================================== */
...
{
    double mean[6][3];  /* [face][axis] raw ADC */
    double sf = dev->accel_sf;

    printf("\n# ╔══════════════════════════════════════════╗\n");
    printf("# ║  6-FACE ACCEL CALIBRATION               ║\n");
    printf("# ║  Put board on ANY 6 different sides,     ║\n");
    printf("# ║  one at a time. Order doesn't matter.    ║\n");
    printf("# ║  Just make sure the board is STILL.      ║\n");
    printf("# ╚══════════════════════════════════════════╝\n");
    ...
    for (int f = 0; f < 6; f++) {
        ...
        /* Auto-detect which axis points up/down: the one with largest |reading| */
    ...
    /* Verify: each axis must have both +1g and -1g faces */
    int bad = 0;
    ...
    for (int a = 0; a < 3; a++) {
        ...
        if (spread < 1.4 * sf) {  /* should be ~2*sf, allow 30% margin */
    ...
    *sz = (float)(2.0 * sf / (raw_max[2] - raw_min[2]));
    ...
    printf("# ╠══════════════════════════════════════════╣\n");
    printf("# ║ OX=%8.1f OY=%8.1f OZ=%8.1f      ║\n", *ox, *oy, *oz);
    printf("# ║ SX=%11.6f SY=%9.6f SZ=%9.6f  ║\n", *sx, *sy, *sz);
    printf("# ╠══════════════════════════════════════════╣\n");
...
{
    /* Critical section: DMA ISR writes ring->head — prevent race
     * between reading head and reading tail (SPSC anti-pattern).
     * IRQ is always enabled when pop is called during acquisition. */
...
/* ========================================================================
 * Online temperature drift calibration — recursive linear regression
 *
 * Model:  gyro_bias = bias_at_T0 + K_T * (T - T0)
 * Method: exponentially-weighted running regression with forgetting factor λ
 *
 * When stationary, collect (temperature, gyro_bias) pairs.
 * Periodically compute linear regression → update K_T and gyro_off_at_T0.
 * ======================================================================== */

#define TC_LAMBDA       0.995f    /* forgetting factor */
#define TC_MIN_SPREAD    3.0f     /* min temperature range (°C) to fit */
#define TC_MIN_SAMPLES   200.0f   /* min effective samples before update */
                         ...
                         float gy_bias, float gz_bias)
{
    ...
    float dt = temp;  /* absolute temperature; slope vs T, not vs ΔT */
    float bias[3] = { gx_bias, gy_bias, gz_bias };
    float lam = tc->lambda;

    /* Exponential decay + accumulate */
...
bool temp_calib_update(temp_calib_t *tc, mpu6050_t *dev)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        /* Linear regression: bias = intercept + slope * T   */
        /* slope = (n·Σxy - Σx·Σy) / (n·Σx² - Σx² + ε)      */
        float denom = n * tc->sum_dt2 - sx * sx;
        if (denom < 0.001f) continue;   /* degenerate data */

        float slope = (n * sxy - sx * sy) / denom;

        /* Update: K_T in °/s per °C, bias at current operating temp */
        dev->K_T[i] = slope;

        /* Recompute bias_at_T0 = current_bias - slope * (T0 - current_T) */
        /* Average of collected bias adjusted to T0 */
        float intercept = (sy - slope * sx) / n;
        /* gyro_off_at_T0 in raw units: convert from °/s to raw */
