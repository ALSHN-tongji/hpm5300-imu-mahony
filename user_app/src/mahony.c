...
/* Gains tuned via parameter sweep (2026-08-07):
 *   Kp/Ki >= 20 is critical for stability on this hardware.
 *   Kp=1.0 was unstable (-3478°/h); Kp=2.0 achieves -1.8°/h.
 *
 * Adaptive Kp (ADAPTIVE_KP=1):
 *   - Detects |a| deviation from 1g → external acceleration
 *   - Smoothly reduces Kp to weaken accel correction during motion
 *   - Paired with adaptive DLPF → dual-layer (hardware+algorithm) optimization
 *   - Set ADAPTIVE_KP=0 for fixed-Kp control experiment */
#define KP_NOMINAL  2.0f    /* max P gain when accel is trustworthy */
...
#define ADAPTIVE_KP 1       /* 1=smoothed adaptive Kp, 0=fixed Kp=KP_NOMINAL */
#if ADAPTIVE_KP
#define KP_SMOOTH_TC 0.15f  /* confidence LP time constant (s) — 150ms */
#endif
...
#if ADAPTIVE_KP
static float conf_smooth = 1.0f;  /* LP-filtered confidence */
#endif
                   ...
                   float dt, mahony_state_t *out)
{
     ...
     * ADAPTIVE_KP=1: LP-smoothed confidence → gradual gain change, no jitter.
     * ADAPTIVE_KP=0: raw per-frame confidence → instant response. */
...
#if ADAPTIVE_KP
    {
        float alpha = dt / (KP_SMOOTH_TC + dt);
        conf_smooth += alpha * (conf - conf_smooth);
    }
    float Kp = KP_NOMINAL * conf_smooth;
#else
